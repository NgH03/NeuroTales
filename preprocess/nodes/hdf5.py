...
class Replay(Node):
    ...
    def __init__(self, filename, keys, speed=1, timespan=None, resync=True, start=0):
        ...
        for key in keys:
            try:
                ...
                # 方法1：基于总时间范围和样本数计算平均采样率
                total_duration = (last - first).total_seconds()
                avg_sampling_rate = nrows / total_duration if total_duration > 0 else 0

                # 方法2：基于前N个样本计算瞬时采样率（更精确）
                sample_size = min(1000, nrows)
                sample_data = self._store.select(key, start=0, stop=sample_size)

                # 计算时间间隔（单位：秒）
                time_deltas = (sample_data.index[1:] - sample_data.index[:-1]).total_seconds()

                # 计算平均时间间隔和采样率
                if len(time_deltas) > 0:
                    avg_interval = np.mean(time_deltas)
                    inst_sampling_rate = 1.0 / avg_interval if avg_interval > 0 else 0
                else:
                    inst_sampling_rate = 0

                # 优先使用瞬时采样率，如果无效则使用平均采样率
                sampling_rate = inst_sampling_rate if inst_sampling_rate > 0 else avg_sampling_rate
                self.logger.debug(f"采样率: {round(sampling_rate)}")
    ...
    def update(self):
        ...
        for key, source in self._sources.items():
            ...
            getattr(self, source["name"]).data["taskId"] = self.filename
